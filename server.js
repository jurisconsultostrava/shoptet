...
async function fetchStoneXJsonRows() {
  ...
  for (const metal of STONEX_METALS) {
    // 1) Nejdřív stáhni množinu kódů, co jsou SKLADEM (in_stock filtr — ověřeně funguje).
    const inStockCodes = new Set();
    {
      ...
      do {
        ...
        for (const p of cat.products || []) {
          const c = String(p.part_number || '').trim();
          if (c) inStockCodes.add(c);
        }
        last = cat.paginator?.last_page || 1;
        page++;
      } while (page <= last);
    }
    // 2) Pak stáhni CELÝ katalog (i vyprodané) a dostupnost urči podle členství v in_stock množině.
    let page = 1, last = 1;
    do {
      ...
      for (const p of cat.products || []) {
        ...
        if (!code) continue;
        const inStock = inStockCodes.has(code);  // je tenhle kód v seznamu skladových?
        rows.push({
          ...
          metalId: metal.id,
          availability: inStock ? 1 : 0, inStock,
