...
function isInFlexiCategory(block) {
  ...
  return ids.some(id => FLEXI_CATEGORY_IDS.includes(id));
}
...
function updateXmlByCode(originalXml, updates, dropUnmatched = false) {
  ...
  return originalXml.replace(/<SHOPITEM\b[\s\S]*?<\/SHOPITEM>/gi, rawBlock => {
    // Auto-zalozeni FLEXI varianty: produkt v cilove kategorii bez variant -> preklopit na variantni.
