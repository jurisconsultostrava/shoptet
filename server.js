...
// CLASSIC varianta (kód bez -FLEXI) dostane vypočtenou cenu (basePrice).
// FLEXI varianta (kód končí -FLEXI / /FLE) dostane basePrice se slevou FLEXI_DISCOUNT_PERCENT.
// Vrací { block, changed } — pokud blok nemá <VARIANTS>, vrací změnu false.
function repriceVariants(block, basePrice) {
  if (!/<VARIANTS>/i.test(block) || basePrice == null) return { block, changed: false };
  ...
  const out = block.replace(/<VARIANT\b[\s\S]*?<\/VARIANT>/gi, vBlock => {
    ...
    const target = isFlexiCode(vCode) ? flexiPrice : Math.round(basePrice);
    changed = true;
    return replaceTag(vBlock, 'PRICE', String(target), true);
  });
